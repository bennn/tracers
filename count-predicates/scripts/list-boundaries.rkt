...
(define (collapse-boundary* boundary*)
  ...
  (define from (boundary-from-file (car boundary*)))
  (define to   (boundary-to-file   (car boundary*)))
...
(define (parse-data fname*)
  (define fname
    (or
     ...
     (for/first ([fname (in-list fname*)] #:when (string-suffix? fname ".rktd")) fname)
     (error 'list-boundaries (format "Expected a .rktd file, got '~a' on command line" fname*))))
